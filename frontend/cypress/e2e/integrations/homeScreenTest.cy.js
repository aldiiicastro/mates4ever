...
describe('Initial pets', () => {
    it('Empty list pets', () => {
         //   cy.intercept('GET', '/api/user/email/aldana@gmail.com', {fixture: 'user_login_data.json'}).as('gets')
            cy.visit('http://localhost:19006/')
            cy.wait(6000)
            cy.get('[data-testid="inputContraseña"]').contains('algo')
        })
    // it('Empty list pets', () => {
    //     cy.intercept('GET', '/api/pet/search?query=', {fixture: 'empty_data.json'}).as('gets')
    //     cy.visit('http://localhost:19006/')
    //     cy.wait('@gets', {timeout: 6000})
